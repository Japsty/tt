...
// RolesAddCtx describes the context to add role to
...
func RolesAdd(ctx RolesAddCtx) error {
	...
	if orchestratorType == replicaset.OrchestratorCartridge {
		if ctx.ReplicasetName == "" {
			return fmt.Errorf("in cartridge need to specify replica set name via --replicaset flag")
		}
	}
	...
	err = orchestrator.RolesAdd(replicaset.RolesAddCtx{
