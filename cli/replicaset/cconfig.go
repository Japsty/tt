...
// orchestrator.
func (c *CConfigInstance) RolesAdd(ctx RolesAddCtx) error {
...
func (c *CConfigApplication) RolesAdd(ctx RolesAddCtx) error {
	replicasets, err := c.Discovery(UseCache)
	if err != nil {
		return fmt.Errorf("failed to get replicasets: %w", err)
	}
...
func (c *CConfigApplication) rolesAdd(ctx RolesAddCtx) (bool, error) {
	if len(c.runningCtx.Instances) == 0 {
		return false, fmt.Errorf("there is no running instances")
	}
	...
	if err := patchLocalCConfig(
		...
		c.publishers,
		func(config *libcluster.Config) (*libcluster.Config, error) {
			return patchCConfigAddRole(config, pRoleTarget)
		},
	); err != nil {
		return false, err
	}
...
func patchCConfigAddRole(config *libcluster.Config,
