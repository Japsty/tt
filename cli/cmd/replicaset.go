...
func newRolesCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "roles",
		Short: "Adds or removes roles for the Cartridge and the Tarantool 3 orchestrator",
	}
...
func NewRolesAddCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Adds a role for the Cartridge and the Tarantool 3 orchestrator",
		Long:  "Adds a role for the Cartridge and the Tarantool 3 orchestrator",
		Run: func(cmd *cobra.Command, args []string) {
			cmdCtx.CommandName = cmd.Name()
			err := modules.RunCmd(&cmdCtx, cmd.CommandPath(), &modulesInfo,
...
func replicasetFillCtx(cmdCtx *cmdcontext.CmdCtx, ctx *replicasetCtx, args []string,
	...
	if err := running.FillCtx(cliOpts, cmdCtx, &ctx.RunningCtx, args); err == nil {
		...
		// In case of adding role when user may to not provide instance.
		if cmdCtx.CommandName == "add" && ctx.InstName == "" {
			if len(ctx.RunningCtx.Instances) == 0 {
				return fmt.Errorf("there is no running instances")
			}
